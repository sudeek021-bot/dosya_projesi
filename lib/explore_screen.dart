...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  static const int _visibleCategoryCount = 7;
  ...
  Future<void> _loadCategoryUsage() async {
    ...
    final Set<String> keys = preferences
        .getKeys()
        .where(
          (String key) =>
          key.startsWith("category_usage_"),
    )
        .toSet();

    for (final String key in keys) {
      final String categoryName = key.replaceFirst(
        "category_usage_",
        "",
      );
  ...
  Future<void> _increaseCategoryUsage(
    ...
    final int newValue =
        (_categoryUsage[categoryName] ?? 0) + 1;

    _categoryUsage[categoryName] = newValue;
  ...
  get _visibleCategories {
    ...
    sorted.sort(
        ...
        final int firstUsage =
            _categoryUsage[firstName] ?? 0;

        final int secondUsage =
            _categoryUsage[secondName] ?? 0;

        if (firstUsage != secondUsage) {
          return secondUsage.compareTo(firstUsage);
        }
    ...
    if (selected != "Tümü") {
      final int selectedIndex = sorted.indexWhere(
      ...
      if (selectedIndex > 0) {
        ...
        sorted.removeAt(selectedIndex);

        sorted.insert(0, selectedItem);
      }
    }

    return sorted
        .take(_visibleCategoryCount)
        .toList();
  }
  ...
  Future<void> _selectCategory(
    ...
    await _increaseCategoryUsage(categoryName);
    await _loadNotes();
  }
  ...
  Widget _allCategoryTile({
    ...
    return Container(
      ...
      child: ListTile(
        onTap: () {
          Navigator.of(modalContext).pop(name);
        },
        leading: Container(
          width: 41,
          height: 41,
  ...
  IconData _categoryIcon(String category) {
    ...
    if (lowerCategory.contains("bilgisayar") ||
        lowerCategory.contains("yazılım")) {
      return Icons.computer_rounded;
    }

    if (lowerCategory.contains("mühendis")) {
      return Icons.engineering_rounded;
    }

    if (lowerCategory.contains("hukuk")) {
      return Icons.gavel_rounded;
    }

    if (lowerCategory.contains("tıp") ||
        lowerCategory.contains("sağlık")) {
      return Icons.medical_services_rounded;
    }

    if (lowerCategory.contains("diş")) {
      return Icons.health_and_safety_rounded;
    }

    if (lowerCategory.contains("eczac")) {
      return Icons.medication_rounded;
    }

    if (lowerCategory.contains("işletme") ||
        lowerCategory.contains("iktisat")) {
      return Icons.business_center_rounded;
    }

    if (lowerCategory.contains("eğitim")) {
      return Icons.school_rounded;
    }

    if (lowerCategory.contains("psikoloji")) {
      return Icons.psychology_rounded;
    }

    if (lowerCategory.contains("mimarlık")) {
  ...
  Widget _searchField() {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: "Not, ders veya üniversite ara...",
