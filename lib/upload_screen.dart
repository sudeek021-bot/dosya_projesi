...
class _UploadScreenState extends State<UploadScreen> {
  ...
  List<Map<String, dynamic>> _universiteSonuclari = [];
  ...
  bool _kategoriHatasiVar = false;

  bool _universiteAraniyor = false;
  bool _universiteListesiAcik = false;
  bool _notYukleniyor = false;

  Timer? _aramaTimer;

  final List<String> _egitimTurleri = [
    "Ön Lisans",
    "Lisans",
    "Yüksek Lisans",
    "Doktora",
  ...
  final Map<String, List<String>> _seviyeler = {
    "Ön Lisans": [
      "Hazırlık",
      "1. Sınıf",
      "2. Sınıf",
  ...
  void initState() {
    ...
    _priceController.addListener(_fiyatDegisti);
    _kategorileriGetir();
  }
  ...
  void dispose() {
    ...
    _priceController.removeListener(_fiyatDegisti);
  ...
  List<String> get _aktifSeviyeler {
    ...
    return _seviyeler[_secilenEgitimTuru] ?? [];
  }

  double get _brutFiyat {
    String text = _priceController.text.trim();
  ...
  double get _komisyon => _brutFiyat * 0.20;

  double get _netKazanc => _brutFiyat - _komisyon;
  ...
  void _universiteAramasiDegisti(String value) {
    _secilenUniversiteId = null;
    _secilenUniversiteAdi = null;
    ...
    if (query.length < 2) {
      setState(() {
        ...
        _universiteListesiAcik = false;
        _universiteAraniyor = false;
      });
    ...
    _aramaTimer = Timer(
      const Duration(milliseconds: 450),
          () async {
        final List<Map<String, dynamic>>? result =
        await _apiService.searchUniversities(
          query,
        );
        ...
        setState(() {
          _universiteSonuclari = result ?? [];
          _universiteListesiAcik = true;
          _universiteAraniyor = false;
        });
      },
    );
  ...
  void _universiteSec(
    ...
    setState(() {
      _secilenUniversiteId = id;
      _secilenUniversiteAdi = name;
  ...
  Future<void> _formuGonder() async {
    ...
    try {
      ...
      await _apiService.uploadNote(
        filePath: _secilenDosya!.path!,
        fileName: _secilenDosya!.name,
        title: _titleController.text,
        description:
        _descriptionController.text,
        categoryId: _secilenKategoriId!,
        universityId:
        _secilenUniversiteId!,

        // Ders arama sistemi sonraki aşamada eklenecek.
        courseId: 1,

        educationType:
        _secilenEgitimTuru!,
        gradeLevel: _secilenSeviye!,
        price: _brutFiyat,
        userId: userId,
      );
  ...
  void _formuTemizle() {
    ...
    setState(() {
      ...
      _secilenKategoriAdi = null;

      _secilenEgitimTuru = null;
      _secilenSeviye = null;
  ...
  String _paraFormatla(double value) {
    final String fixed =
  ...
  Widget _universiteSonucKutusu() {
    return Container(
      ...
      child: _universiteSonuclari.isEmpty
          ? const Padding(
        padding:
        EdgeInsets.all(18),
        child: Text(
